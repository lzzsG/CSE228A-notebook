...
3. **更新逻辑**：
   ...
   - 通过``Mux(io.en, nextVal, count)`控制使能信号：
...
- Chisel 使用后缀`.U`、`.S` 和`.B` 对字面值进行显式转换：
